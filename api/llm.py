...
# ----------------------------------------------------------------------------
# Research triple extraction
# ----------------------------------------------------------------------------


EXTRACT_TRIPLES_PROMPT = """You extract biomedical knowledge-graph triples from a research abstract, in service of a user's topic of interest.
...
Extract 3-10 subject-predicate-object triples that capture the paper's concrete findings. Rules:
- subject and object are concise entity names (genes, proteins, metabolites, drugs, pathways, processes, diseases) — e.g. "SIRT1", "NAD+", "mitophagy". Do not use pronouns or long phrases.
- predicate is a lowercase snake_case relation such as: activates, inhibits, increases, decreases, promotes, suppresses, produces, deacetylates, binds, regulates, causes, ameliorates, is_a.
- confidence is a number between 0 and 1 reflecting how directly the abstract supports the triple.
- rationale is one sentence citing what the abstract says (no invented details).
- Only extract what the abstract actually supports. Skip background claims.

Respond with ONLY a JSON object, no markdown fences:
{{"triples": [{{"subject": "...", "predicate": "...", "object": "...", "confidence": 0.0, "rationale": "..."}}]}}"""


async def extract_triples(
    topic_query: str, title: str, abstract: str, timeout: float = 120.0
) -> list[dict]:
    """Extract candidate triples from one abstract via the utility agent.

    Returns [] on any failure — extraction is best-effort per paper.
    """
    if not abstract:
        return []
    prompt = (
        EXTRACT_TRIPLES_PROMPT
        .replace("{topic}", topic_query[:500])
        .replace("{title}", title[:300])
        .replace("{abstract}", abstract[:6000])
    )
    session_id = None
    try:
        session_id = await create_session(title="extract-triples")
        text = await _prompt_sync(session_id, prompt, timeout=timeout)
        ...
        for t in triples[:12]:
            ...
            if not (subj and pred and obj_name):
                continue
            try:
            ...
            clean.append({
                "subject": subj[:120],
                "predicate": re.sub(r"\s+", "_", pred.lower())[:60],
                "object": obj_name[:120],
                "confidence": confidence,
                "rationale": str(t.get("rationale", "")).strip()[:1000],
            })
        return clean
    except Exception as e:  # noqa: BLE001 - extraction is best-effort
        logger.error(f"extract_triples failed: {e}")
        return []
    finally:
        if session_id:
            await delete_session(session_id)
