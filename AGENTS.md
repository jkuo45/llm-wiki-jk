...
- **Triples schema (v2)** — `src/**/_triples.json` is multilingual: each triple carries a stable `id`, `context` as a BCP-47 map (`en-US` canonical + `zh-TW` translation), English-only `confidence`/`source_document`, and `created`/`updated` ISO-8601 UTC timestamps. Author `en-US` then translate to `zh-TW` in the same pass (preserve entity names / gene symbols); keep `id`/`created` stable and bump `updated` on change. Run the idempotent `scripts/normalize_triples_schema.py` after edits (fills ids/timestamps, validates, and `--check` reports missing `zh-TW`). To bulk-fill translations for an untranslated corpus, run the idempotent, resumable `scripts/backfill_triple_zhTW.py` (drives the read-only `wiki-util` opencode agent; returns only triples still missing `zh-TW`), then re-run the normalizer and rebuild. The rebuild resolves node descriptions / edge dedupe as most-recent-`updated` wins.
- **Graphify updates** use `scripts/03_rebuild_from_triples.py` (the vault's canonical rebuild from per-topic `src/**/_triples.json`), NOT standard graphify ingest. Run it to regenerate `graphify-out/` (graph.json, GRAPH_REPORT.md, labels, html) for analysis, and regenerate the runtime data JSONs in `web/data/` (nodes/edges/legend.json, copies of graph.json/manifest.json, and version.json with a content-hash cache tag the app uses to cache-bust its fetches). The web app files themselves (`web/index.html`, `three-graph.css`, `components/`, `pages/`) and the crawler/LLM files at the web root (`llms.txt`, `robots.txt`, `sitemap.xml`) are hand-maintained/committed — this script only ever writes under `graphify-out/` and `web/data/`. `web/data/query.json`, `web/data/translations-zh-TW.json`, and `web/data/predicates-zh-TW.json` are hand-maintained alongside the generated files — update translation files as needed. The web app sources entity summaries for tooltips/modals directly from graph.json node `description` fields; `description_zh_TW` carries the zh-TW context (wiki-context.json is retired).
- **Notes panel tags** — `src/images/manifest.json` (the image-ingest notes manifest, distinct from the graphify `web/data/manifest.json`) uses a **closed** tag vocabulary enforced by `scripts/normalize_manifest_vocab.py` (canonical, idempotent). Format: first tag = **TOPIC**, then **FORMAT** facets (`notes`, `diagrams-charts`, `graph-analysis`, `generated-image`) and **CONTENT** facets — ≤12 tags total. `notes` = the handwritten corpus only; don't tag printed infographics that way (use `diagrams-charts`). Legacy tags are banned (`photo`, `paper-figure`, `bilingual`, `speculative`, `comparison`, `misc`). Bilingual tag labels for the web panel live in `web/data/notes-tags-zh-TW.json` — hand-maintained, and it must cover the whole vocabulary (`web/components/notes.js` renders slugs via `tagLabel()`). Transcript hygiene for the same manifest: `scripts/tighten_manifest_transcripts.py` is the idempotent cleanup (strips OCR-model editorial narration, joins line-break-truncated words, normalizes `[illegible]`→`[AMBIGUOUS]`); run it after any ingest/edit. When a handwritten transcript is unusably garbled, re-OCR the source image with the local vision model (`ollama run qwen3.5 --think=false "…" /abs/path` — absolute path required, thinking must be off) and fold via `apply_qwen_reocr.py`, then re-run the tighter.
