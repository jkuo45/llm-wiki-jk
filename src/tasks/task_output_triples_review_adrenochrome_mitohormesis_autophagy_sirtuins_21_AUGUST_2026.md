...
- vs. `Adrenochrome --detected_in--> Rheumatoid synovial fluid` (conf 0.75, *neutrophils role in adrenochrome production*)
...
- Source (*as senotherapeutic agent*) is explicitly hypothetical: "proposes… could permanently inhibit… plausible route." Meanwhile adjacent triples state adrenochrome `induces Oxidative Stress` (0.85) and `modifies IKK complex` (0.75).
...
- Related tension inside the same corpus: `SIRT5 --suppresses_hcc_development_via--> bile acid metabolism` vs `SIRT5 --promotes--> HCC growth and metastasis` — both from *sirtuins in health and disease*. Recommend normalizing both to explicit dual-role framing rather than leaving opposite-polarity edges.
...
- `COMT --predominates_in--> itself` (conf 0.93, comt topic) — same collapse pattern.
- `Val158Met --is--> itself` (conf 0.92, comt topic).
...
- `Rapamycin --induces--> Autophagy` (0.95) + `Rapamycin --activates--> Autophagy` (0.96/0.97 variants across docs)
- `Metformin --activates--> Mitohormesis` (0.8) + `Metformin --exploits--> Mitohormesis` (0.9)
- `Spermidine --induces/enhances--> Autophagy` (two 0.95 edges)
- `Nicotinamide Riboside --increases--> NAD+` + `--converts_to--> NAD+` (both fine, but ensure predicates reflect precursor conversion vs pharmacological elevation)

### T3-2. Lifespan-claim asymmetry worth annotating, not deleting
- `SIRT1 --brain_specific_brasto_overexpression_extends--> Lifespan` alongside `SIRT1 --global_overexpression_fails_to_extend--> Lifespan` (both 0.95, same doc). These are genuinely compatible (tissue-specific vs whole-body) and the predicates encode the distinction, but the compound predicates are fragile for graph tooling — consider splitting qualifier into context.
...
- **No `Resveratrol ↔ Sirtuins controversy` representation**: corpus stores `Resveratrol --activates--> SIRT1` (0.85/0.95, twice) but no capture of the well-known challenge to *direct* activation at physiological doses (e.g., Park et al. 2012; the corpus has no `does_not_activate` edge for this pair even though such negation predicates exist elsewhere). Given resveratrol's 36-degree hub status, add a qualified counter-edge or lower confidence.
- `Mitochondrial ROS --activates--> Autophagy` (0.85, mitohormesis doc) vs `Mitohormesis --requires--> ROS` — compatible but currently unlinked; consider an explicit ROS-mediated chain.
...
- `Autophagy --is_impaired_in--> Senescence` (0.72) — directionally consistent with autophagy-suppresses-SASP (0.9) but phrasing ambiguous ("impaired in" = reduced during senescence? verify).
- `Senescence --can_promote--> Cancer` (0.72) vs `Paracrine Senescence --suppresses_tumor--> Cancer` (0.7) vs `Paracrine Senescence --can_promote--> Cancer` (0.68) — three-way divergence reflecting real literature (immune-clearance-dependent duality); needs unified dual-role note.
- `Adrenochrome --promotes--> Lipid Peroxidation` (0.7), `Adrenolutin --is_a_metabolite_of--> Adrenochrome` (0.7) vs separate `Adrenolutin --derives_from--> Adrenochrome` (0.8) — duplicate relationship with different predicates/confidences.
