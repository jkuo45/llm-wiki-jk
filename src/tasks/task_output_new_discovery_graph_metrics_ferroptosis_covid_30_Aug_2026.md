...
created: 2026-08-30
updated: 2026-08-30
source: counterfactual + staged replay sims on graphify-out/graph.json, wiki-out/wiki-graph.json & web/public/data/edges.json (combined); scripts/analysis/node_analysis.py, scripts/analysis/role_query.py, graphify CLI; web research (Cell, Nature, NEJM, Lancet, eLife, PNAS, FDA 2012-2026)
tags:
  - task-output
  - knowledge-graph
  - graph-metrics
...
**Repo artifacts & tools**: `graphify-out/graph.json` · `wiki-out/wiki-graph.json` · `web/public/data/edges.json` (combined; 3,895 typed edges verified) · `web/public/data/link-prediction.json` · `web/public/data/assumptions.json` (+ applied state: `assumptions-build.json`, GRAPH_REPORT "Assumption State"; canonical selections synced by `07_sync_assumptions.py`) · `scripts/analysis/node_analysis.py` · `scripts/analysis/role_query.py` · `03_triple_lookup.py` · replay/counterfactual harnesses (temp `replay_growth.py`, `sim_new_discovery.py` — promote to `scripts/` if adopted).
