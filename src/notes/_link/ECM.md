---
title: ECM
description: The Extracellular Matrix (ECM) is a complex network of extracellular
  macromolecules, such as collagen, enzymes, and glycoproteins like Fibronectin and
  Vitronectin, that provide structural and bioch...
created: 2024-01-01
updated: 2024-01-01
tags:
  - anatomy
aliases: []
---
# ECM
The Extracellular Matrix (ECM) is a complex network of extracellular macromolecules, such as collagen, enzymes, and glycoproteins like [[Fibronectin]] and [[Vitronectin]], that provide structural and biochemical support to surrounding cells.
---
### Merged from notes/autophagy/ECM.md
The **ECM** (Extracellular Matrix) is a complex network of proteins and carbohydrates that provides structural and biochemical support to surrounding cells, and its remodeling is regulated by [[Lysosome]] activity during processes like [[C. elegans]] molting.


#

# 

## Documents

List of documents that mention this entity

  - [[_document_ - Lysosome biogenesis Regulation and functions|Lysosome biogenesis Regulation and functions]]
    - C) STA-2– and ELT-3–dependent Lysosome biogenesis in ECM remodeling during C. elegans molt.

  - [[_document_ - Cellular Mechanisms and Regulation of Quiescence|Cellular Mechanisms and Regulation of Quiescence]]
    - The niche is comprised of surrounding cells, Extracellular Matrix (ECM), and blood vessels, which interact with quiescent cells using soluble factors or through direct contact (Fiore et al., 2018) (Figure 3).

  - [[_document_ - sirtuins in health and disease s41392-022-01257-8|sirtuins in health and disease s41392-022-01257-8]]
    - EMT is the most well-established example of changes in cell–cell adhesion, which refers to nonepithelial cells that are loosely embedded in an ECM.


## Connections
- [[Fibronectin]] — interacts with ECM
- [[Vitronectin]] — interacts with ECM
- [[Lysosome]] — degradative organelle; autophagic flux endpoint; nutrient sensing hub
- [[C. elegans]] — interacts with ECM
## Linking Summary
- New links added: [[Fibronectin]], [[Vitronectin]], [[Integrins]], [[RGD peptide]], [[Lysosome]]
- Suggested new entity notes to create: [[Collagen]], [[Laminin]]
  - Strong connections to strengthen: [[ECM]] ↔ Integrins, [[ECM]] ↔ [[Lysosome]]
