---
title: Chronic Lymphocytic Leukemia
description: Chronic lymphocytic leukemia (CLL), the most common adult leukemia in Western populations, characterized by accumulation of mature CD5+ B cells; SIRT3-mediated ROS buffering contributes to chemoresistance.
protected: false
created: 2026-08-18
updated: 2026-08-18
tags:
  - disease
  - cancer
  - leukemia
  - hematology
aliases:
  - CLL
  - Chronic lymphatic leukemia
---

# Chronic Lymphocytic Leukemia

**Chronic lymphocytic leukemia (CLL)** is a hematological malignancy characterized by progressive accumulation of morphologically mature but functionally incompetent CD5+ B lymphocytes in the blood, bone marrow, and lymphoid tissues. It is the most common leukemia in adults of Western countries, with a median onset in the seventh decade — making aging a central demographic driver. The clinical course is highly variable, ranging from indolent disease to aggressive progression requiring therapy with targeted agents (BTK inhibitors, BCL2 inhibitors such as venetoclax) or chemoimmunotherapy.

## SIRT3 and chemoresistance

The SIRT3 review highlights a context-dependent, tumor-supportive role of [[SIRT3]] in CLL: SIRT3 enables metabolic adaptation that confers chemoresistance via [[ROS]] buffering[272]. By boosting mitochondrial antioxidant capacity (e.g., through [[MnSOD]] deacetylation and [[IDH2]]-driven [[NADPH]] production), SIRT3 protects leukemic cells from oxidative damage imposed by cytotoxic chemotherapy, allowing metabolic plasticity and survival under treatment pressure. This contrasts with SIRT3's tumor-suppressive actions in other settings and exemplifies the dualistic, context-dependent behavior of SIRT3 in cancer biology.

## Therapeutic implications

The finding argues for context-specific SIRT3 modulation in oncology: while SIRT3 activation is desirable in aging-related degenerative disease, SIRT3 inhibition might sensitize CLL cells to chemotherapy by disabling their ROS-buffering capacity — highlighting the need for tumor-context-aware SIRT3-based strategies.

## Documents

- [[_document_ - Roles of SIRT3 in aging and aging-related diseases|Roles of SIRT3 in aging and aging-related diseases (You & Wang 2025)]]
  - Reports SIRT3-enabled metabolic adaptation and ROS buffering as a chemoresistance mechanism in CLL.

## Connections

- [[SIRT3]]: Enables metabolic adaptation and ROS buffering conferring chemoresistance in CLL.
- [[ROS]]: Oxidative burden buffered by SIRT3 to protect leukemic cells.
- [[MnSOD]]: Key SIRT3 deacetylation target in the antioxidant program.
- [[Cancer]]: Disease category exemplifying SIRT3's context-dependent oncogenic potential.

## Linking Summary

- New links added: [[SIRT3]], [[ROS]], [[MnSOD]], [[Cancer]]
- Suggested new entity notes to create: none
- Strong connections to strengthen:
    - [[SIRT3]] ↔ [[Cancer]] (context-dependent chemoresistance via ROS buffering)

  - No additional entities required.