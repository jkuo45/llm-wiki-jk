---
title: Docetaxel
description: A taxane chemotherapeutic that, like mitoxantrone, induces senescence and chemoresistance in cancer models; its senescence-driven resistance is suppressed by apigenin.
created: 2026-07-10
updated: 2026-07-10
tags:
  - chemical-compound
  - chemotherapeutic
  - taxane
  - senescence
aliases: [Docetaxel, DTX, Taxotere]
---

# Docetaxel

**Docetaxel (DTX)** is a taxane chemotherapeutic that stabilizes microtubules and disrupts mitosis. In the apigenin study it served as an alternative chemotherapeutic (to [[Mitoxantrone|MIT]]) to validate apigenin's ability to counteract senescence-associated chemoresistance.

## Relevance to Apigenin Study

> [!info] Source: [[_document_ - repurposing_apigen_senomorphic.09.09.611999v1.full|Repurposing apigenin for senomorphic effect in antiaging pipelines]]
> Replacing MIT with docetaxel largely phenocopied the MIT result: senescent stromal cell-derived conditioned media increased cancer cell apoptosis-resistance and chemoresistance, and both effects were considerably suppressed by [[Apigenin]].

## Connections

- [[Mitoxantrone]] — Alternative chemotherapeutic comparator.
- [[Apigenin]] — Suppresses docetaxel-associated senescence chemoresistance.
- [[Chemoresistance]] — Phenotype reversed by apigenin.
- [[Senescence]] — Docetaxel can induce therapy-associated senescence.

## Linking Summary

- New links added: [[Mitoxantrone]], [[Apigenin]], [[Chemoresistance]], [[Senescence]]
- Suggested new entity notes to create: (none)
- Strong connections to strengthen: [[Docetaxel]] ↔ [[Chemoresistance]]
