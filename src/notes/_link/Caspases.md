---
title: Caspases
description: Caspases are a family of cysteine proteases that play essential
  roles in Apoptosis, Inflammation, and Pyroptosis. They are synthesized as inactive
  zymogens (procaspases) and activated by proteo...
created: 2026-07-04
updated: 2026-09-02
tags:
  - enzyme
  - apoptosis
aliases: [Caspase, Cysteine-aspartic proteases]

---

# Caspases
**Caspases** are a family of cysteine proteases that play essential roles in [[Apoptosis|programmed cell death (apoptosis)]], [[Inflammation]], and [[Pyroptosis]]. They are synthesized as inactive zymogens (procaspases) and activated by proteolytic cleavage.

#

# 

## Documents

List of documents that mention this entity

  - [[_document_ - Evading apoptosis in cancer|Evading apoptosis in cancer]]
    - It involves a series of cellular events that ultimately leads to activation of a family of cysteine proteases called Caspases. In response to various apoptotic stimuli, “initiator” Caspases (Caspase-2, Caspase-8, Caspase-9, or Caspase-10) are activated.

  - [[_document_ - sirtuins in health and disease s41392-022-01257-8|sirtuins in health and disease s41392-022-01257-8]]
    - plays a central role in the prevalence of diseases related to Apoptosis.[–] SIRT1 regulates p53 deacetylation, which is associated with the Apoptosis-inhibiting signaling pathway, mainly including the p53-induced death domain protein Pidd, p21, Bax/Bad and ...


## Post-Translational Regulation

> [!important] Phosphorylation Controls Caspase Activity
> Individual caspases are subject to regulatory phosphorylation that fine-tunes apoptotic sensitivity:
> - **[[Caspase-8]]**: Phosphorylated by [[Src]] at Tyr380 (inhibits activation); dephosphorylated by [[SHP1]] (restores function)
> - **[[Caspase-7]]**: Phosphorylated by [[PAK2]] at Ser30/Thr173/Ser239 (inhibits activation and substrate binding)
> - **[[Caspase-9]]**: Phosphorylated by [[Akt]] at Ser196 (suppresses activity)
> - **[[XIAP]]**: Directly binds and inhibits [[Caspase-3]], [[Caspase-7]], and [[Caspase-9]]; regulated by [[TBK1]] (degradation) and [[Akt]] (stabilization)

## Sex Differences — The Female-Preferred Death Effectors

> [!important] Caspases are the female-preferred cell-death effectors
> In the sex-difference cell-death framework (detailed in [[Apoptosis]]), cells of female origin preferentially execute **caspase-dependent apoptosis**, whereas male cells preferentially die via [[PARP1]]/AIF-dependent necrosis. Caspases are thus the XX-arm of the death machinery, and this has direct therapeutic consequences: pan-caspase inhibition protects females but not males in experimental stroke.

- **Caspase-3/8 activation is higher and earlier in female (XX) cells and neurons** after ischemia; [[Caspase-8]] shows ~3× greater activation with nuclear translocation unique to XX cells (Liu et al., *Stroke* 2009; Sharma et al., *ASN Neuro* 2011).
- **XY cells** favor [[PARP1]] overactivation → [[NAD+]] depletion → energy failure → AIF-mediated necrosis, largely bypassing the caspases (see [[NAD+]] for the PARP-1 sex divergence).
- Cross-linked with [[p53]] (which sex-specifically biases the apoptosis-vs-senescence decision) and [[Bcl-2]] (the estrogen-driven anti-apoptotic setpoint that raises the female apoptotic threshold) — see the unified framework across [[Apoptosis]] ↔ [[Caspases]] ↔ [[p53]] ↔ [[Bcl-2]].

## Connections
- [[Apoptosis]] — Executioner caspases (3, 6, 7) drive apoptotic cell death
- [[Inflammation]] — Caspase-1 activates IL-1β and IL-18
- [[Oxidative Stress]] — Can activate caspase-dependent apoptosis
- [[Caspase-8]] — Initiator caspase of the extrinsic pathway
- [[Caspase-9]] — Initiator caspase of the intrinsic pathway
- [[Caspase-3]] — Primary executioner caspase
- [[Caspase-7]] — Executioner caspase regulated by PAK2
- [[XIAP]] — Direct inhibitor of caspases 3, 7, and 9
- [[PARP1]] — the opposing male-preferred (XY) death effector
- [[p53]] — sex-hormone-regulated upstream decision point for the death program
- [[Bcl-2]] — estrogen-driven anti-apoptotic counterweight

## Linking Summary
- New links added: [[Apoptosis]], [[Inflammation]], [[Pyroptosis]], [[Oxidative Stress]], [[Caspase-8]], [[Caspase-9]], [[Caspase-3]], [[Caspase-7]], [[XIAP]], [[SHP1]], [[PAK2]], [[Akt]], [[TBK1]]
- Suggested new entity notes to create: [[Inflammasome]], [[Apaf-1]]
  - Strong connections to strengthen: [[Caspases]] ↔ [[Apoptosis]], [[Caspases]] ↔ [[XIAP]]

## Linking Summary (Sex Differences — Cell-Death Cross-Link, 2026-09-02)

- **New Sex Differences section** added cross-referencing [[Apoptosis]], [[p53]], [[Bcl-2]], and [[PARP1]] under the unified XX-caspase / XY-PARP-1-AIF framework.
- Cross-links added: [[PARP1]], [[p53]], [[Bcl-2]], [[NAD+]], [[Caspase-8]], [[Caspase-3]].
- Strong connections to strengthen: [[Caspases]] ↔ [[Apoptosis]] (XX-caspase arm), [[Caspases]] ↔ [[PARP1]] (opposing XY arm).
