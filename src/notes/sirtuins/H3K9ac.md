---
title: H3K9ac
description: Acetylation of histone H3 at lysine 9 (H3K9ac) — an euchromatic, transcription-permissive mark that is removed by the sirtuins SIRT1 and SIRT6 to impose gene silencing, suppress NF-κB, and facilitate DNA repair.
protected: false
created: 2026-07-14
updated: 2026-07-14
tags:
  - histone-modification
  - epigenetic-mark
  - sirtuin-substrate
  - chromatin
url: #
source: #
aliases:
  - Histone H3 lysine 9 acetylation
  - H3 lysine 9 acetylation
  - Acetyl-H3K9
---

# H3K9ac

**H3K9ac** denotes the acetylation of lysine 9 on the N-terminal tail of **histone H3**. As a euchromatic mark, H3K9ac neutralizes the positive charge of lysine, loosens nucleosome–DNA contacts, and is generally associated with open, transcriptionally permissive chromatin. Its removal (deacetylation) by NAD⁺-dependent sirtuins is a central mechanism by which sirtuins impose transcriptional silencing and reshape the epigenetic landscape.

## Regulation by Sirtuins

- **SIRT1:** Deacetylates histone H3 at lysine 9 (and K79), promoting tightly compacted chromatin and gene silencing — a hallmark of SIRT1 activity at telomeres, promoters (e.g., of [[AR]], [[BRCA1]], [[EZH2]], [[EP300]]), and in dynamic chromatin association of DNA-repair factors.
- **SIRT6:** A principal H3K9ac (and H3K56ac) deacetylase. SIRT6 deacetylates H3K9 at the promoters of [[NF-κB|NF-κB]] target genes, repressing inflammatory transcription; at [[ERK|ERK1/2]] promoters, inhibiting ERK expression; and at the [[Nkx3.2]] promoter, relieving repression of [[GATA5]]. H3K9 deacetylation also facilitates recruitment of DSB-repair factors ([[53BP1]], [[BRCA1]]) to damage-flanking chromatin.
- **SIRT6 activators** such as [[UBCS039]]/[[MDL-800]] and [[Cyanidin]] decrease both H3K9ac and H3K56ac, compacting chromatin and promoting DNA double-strand break repair.

## Physiological & Pathological Relevance

- **Inflammation:** SIRT6-mediated H3K9 deacetylation at NF-κB promoters underlies its anti-inflammatory effect and organismal lifespan regulation (Kawahara et al., 2009).
- **Genome stability & senescence:** SIRT6 loss leads to telomeric H3K9 hyperacetylation and senescence; SIRT6 preserves telomere integrity.
- **Viral replication:** SIRT3 and SIRT6 reduce H3K9 acetylation on HBV covalently closed circular DNA, inhibiting viral transcription.
- **Cancer & fibrosis:** SIRT6 activators that lower H3K9ac/H3K56ac induce autophagy and suppress myofibroblast activation.

## Connections

  - [[Histone H3]]: The parent histone carrying the K9 mark.
  - [[SIRT1]], [[SIRT6]]: Major sirtuin deacetylases of H3K9ac.
  - [[NF-κB]]: NF-κB target promoters deacetylated by SIRT6 at H3K9.
  - [[53BP1]], [[BRCA1]]: DSB-repair factors recruited after SIRT6-mediated H3K9 deacetylation.
  - [[UBCS039]], [[MDL-800]], [[Cyanidin]]: SIRT6 activators that lower H3K9ac.
  - [[Nkx3.2]], [[GATA5]], [[ERK]]: Loci under H3K9ac-dependent control.

## Documents

  - [[_document_ - sirtuins (resveratrol), gemini|Sirtuins and Resveratrol (Gemini)]]
    - States SIRT6 activation rapidly deacetylates H3K9ac and H3K56ac, compacting chromatin and recruiting DSB-repair machinery.
  - [[_document_ - centenarian_SIRT6 nihpp-rs9997679v1|Centenarian SIRT6 (rs9997679)]]
    - CENT hMSCs show increased H3K9 acetylation, consistent with reduced deacetylase activity of CentSIRT6.
  - [[_document_ - SIRT1 Protein roles at the nexus of health, disease, and therapeutics|SIRT1 Protein Roles]]
    - Describes SIRT1 deacetylation of H3 at lysine 9 and K79, promoting compact chromatin and gene silencing.

## Linking Summary

- New links added: [[Histone H3]], [[SIRT1]], [[SIRT6]], [[NF-κB]], [[53BP1]], [[BRCA1]], [[UBCS039]], [[MDL-800]], [[Cyanidin]], [[Nkx3.2]], [[GATA5]], [[ERK]], [[AR]], [[BRCA1]], [[EZH2]], [[EP300]], [[MDL-800]]
- Suggested new entity notes to create: [[H3K56ac]], [[H3K27ac]]
- Strong connections to strengthen: [[H3K9ac]] ↔ [[SIRT6]]; [[H3K9ac]] ↔ [[NF-κB]]
