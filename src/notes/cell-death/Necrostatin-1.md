---
title: Necrostatin-1
description: A small-molecule inhibitor of necroptosis that targets receptor-interacting protein kinase 1 (RIPK1).
protected: true
created: 2026-07-17
updated: 2026-09-14
tags:
  - chemical-compound
  - cell-death
aliases: [Nec-1, Necrostatin 1, 5-(1H-Indol-3-ylmethyl)-3-methyl-2-thioxo-4-imidazolidinone]
---

# Necrostatin-1

**Necrostatin-1** (Nec-1) is a selective small-molecule inhibitor of [[RIPK1|receptor-interacting protein kinase 1]] (RIPK1), a key signaling node in the [[Necroptosis|necroptosis]] pathway. It is the most widely used tool compound for distinguishing necroptosis from other forms of regulated cell death.

## Mechanism of Action

Nec-1 allosterically inhibits RIPK1 kinase activity, preventing the formation of the necrosome (RIPK1–RIPK3–MLKL complex) that executes necroptotic cell death. Nec-1 does not affect apoptosis (caspase-dependent) or ferroptosis (lipid peroxidation-dependent), making it a specific pharmacological probe for necroptosis.

## Research Applications

- Cell death mechanism dissection in combination with [[Z-VAD-FMK]] (caspase inhibitor) and [[Ferrostatin-1]] (ferroptosis inhibitor)
- In vivo protection in models of [[Ischemia-reperfusion Injury]], [[Myocardial infarction]], [[Stroke]], and [[Neurodegenerative Diseases]]
- Validating RIPK1-dependent signaling in [[Inflammation]] and [[Cytokine]] release

> [!info]
> Source: [[_document_ - Necroptosis a regulated inflammatory mode of cell death|Dhuriya & Sharma 2018]]
> Nec-1 acts by blocking [[RIPK1]] phosphorylation and translocation, disrupting downstream necrosome signaling; with zVAD-fmk it is complementary across neurodegeneration models (AD: reduced Aβ/tau, improved cognition in APP/PS1; PD: survival in 6-OHDA PC12; HD: ST14A survival). After TBI it also engages Akt/mTOR anti-apoptotic/autophagy effects, and in SOD1-G93A ALS (as 7-Cl-O-Nec-1) it reduces axonal degeneration and motor dysfunction.

## Documents

- [[_document_ - Necroptosis a regulated inflammatory mode of cell death|Dhuriya & Sharma 2018 review]]
  - Mechanism (phosphorylation/translocation block), AD/PD/HD Table 1, TBI Akt/mTOR off-target note, SOD1-G93A ALS rescue.
- [[_document_ - Regulatory complexity and therapeutic targeting of the necroptosis network|Niu et al. 2026 Front Immunol]]
  - Necrostatin-1 as the reference RIPK1 inhibitor in the therapeutic landscape, alongside RIPK3 (GSK'840/GSK'872/Zharp-99) and MLKL (necrosulfonamide) inhibitors; context-dependent duality of inhibiting vs inducing necroptosis.

## Connections

- [[Necroptosis]] — Nec-1 is a defining inhibitor of this pathway
- [[RIPK1]] — Direct molecular target
- [[Z-VAD-FMK]] — Used in combination for cell death mechanism dissection
- [[Ferrostatin-1]] — Complementary ferroptosis inhibitor for cell death panel
- [[Apoptosis]] — Nec-1 does not inhibit caspase-dependent apoptosis
- [[Ferroptosis]] — Nec-1 does not inhibit lipid peroxidation-dependent death
- [[Adrenochrome]] — Nec-1 could be used in a rescue panel to rule out necroptosis in adrenochrome toxicity models

## Linking Summary

- New links added: [[Necroptosis]], [[RIPK1]], [[Z-VAD-FMK]], [[Ferrostatin-1]], [[Apoptosis]], [[Ferroptosis]], [[Adrenochrome]]
- Strong connections to strengthen: [[Necrostatin-1]] ↔ [[RIPK1]], [[Necrostatin-1]] ↔ [[Necroptosis]]
- Source enrichment (2026-09-14): [[_document_ - Regulatory complexity and therapeutic targeting of the necroptosis network|Niu et al. 2026]] — Nec-1 in the direct-inhibitor landscape of the necroptosis network. New links: [[GSK872]].
