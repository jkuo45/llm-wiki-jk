...
def test_create_topic(client, fake_db):
...
def test_create_topic_with_unknown_graph_rejected(client, fake_db):
...
def test_list_topics_scoped_to_owner(make_client, fake_db):
...
def test_search_run_stores_records_and_triples(client, research_env, fake_db):
...
def test_search_run_is_idempotent_on_rerun(client, research_env, fake_db):
    tid = research_env["topic_id"]
...
def test_search_run_failed_adapter_is_isolated(client, research_env, fake_db, monkeypatch):
...
def test_search_run_failure_marks_topic_failed(client, research_env, fake_db, monkeypatch):
...
def test_search_unknown_sources_fall_back(client, research_env, fake_db):
    r = _run_search(client, research_env["topic_id"], sources=["not-a-source"])
    assert r.json()["sources"] == ["pubmed", "europepmc"]


def test_extract_limit_zero_skips_llm(client, research_env, fake_db, monkeypatch):
    calls = []
...
def test_topic_detail_includes_results_and_triples(client, research_env, fake_db):
    tid = research_env["topic_id"]
    _run_search(client, tid)
...
def test_approve_links_to_base_entities(client, research_env, fake_db):
    tid = research_env["topic_id"]
...
def test_approve_unknown_label_creates_custom_node(client, research_env, fake_db):
    tid = research_env["topic_id"]
    triple = _pending_triple(fake_db, tid, subject="Novel Thing X", object_="SIRT1")
    client.post(f"/v1/research/triples/{triple['id']}/review", json={"status": "approved"})

    custom = [n for n in fake_db.all("user_nodes") if n.get("custom_label")]
    assert [n["custom_label"] for n in custom] == ["Novel Thing X"]


def test_approve_twice_rejected(client, research_env, fake_db):
    tid = research_env["topic_id"]
    triple = _pending_triple(fake_db, tid)
    client.post(f"/v1/research/triples/{triple['id']}/review", json={"status": "approved"})
    r = client.post(f"/v1/research/triples/{triple['id']}/review",
...
def test_reject_flow(client, research_env, fake_db):
    tid = research_env["topic_id"]
    triple = _pending_triple(fake_db, tid)
    r = client.post(f"/v1/research/triples/{triple['id']}/review",
    ...
    row = next(t for t in fake_db.all("extracted_triples") if t["id"] == triple["id"])
    assert row["status"] == "rejected"


def test_review_requires_topic_ownership(client, make_client, fake_db, seeded_entities):
...
def test_review_with_graph_override(client, research_env, fake_db):
    tid = research_env["topic_id"]
    ...
    r = client.post(f"/v1/research/triples/{triple['id']}/review",
                    json={"status": "approved", "user_graph_id": research_env["graph_id"]})
    assert r.status_code == 200


def test_invalid_review_status_rejected(client, research_env, fake_db):
...
def test_search_other_users_topic_404(client, make_client, fake_db):
    topic = make_client(USER_B).post("/v1/research/topics", json={"query": "bb"}).json()
