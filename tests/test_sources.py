...
@pytest.mark.asyncio
async def test_pubmed_search_parses_articles(patch_transport):
    seen = patch_transport(_pubmed_handler(["111", "222"]))
...
@pytest.mark.asyncio
async def test_pubmed_year_fallback_and_missing_abstract(patch_transport):
...
@pytest.mark.asyncio
async def test_pubmed_search_empty_results(patch_transport):
    patch_transport(_pubmed_handler([]))
    assert await PubmedAdapter().search("x", 5) == []


@pytest.mark.asyncio
async def test_pubmed_fetch_single(patch_transport):
...
@pytest.mark.asyncio
async def test_europepmc_search_parses_and_strips_markup(patch_transport):
    def handler(request: httpx.Request) -> httpx.Response:
...
@pytest.mark.asyncio
async def test_europepmc_fetch_by_id(patch_transport):
    def handler(request: httpx.Request) -> httpx.Response:
...
@pytest.mark.asyncio
async def test_europepmc_no_hits(patch_transport):
    patch_transport(lambda r: httpx.Response(200, json={"resultList": {"result": []}}))
...
def test_record_as_row_shape():
    ...
    for key in ("title", "abstract", "authors", "pub_year", "venue", "url", "doi", "raw"):
        assert key in row
