...
def test_create_plain_graph(client):
...
def test_create_fork_seeds_topic_entities(client, seeded_entities):
...
def test_create_fork_unknown_topic_rejected(client, fake_db):
...
def test_db_unavailable_returns_503(client, fake_db, monkeypatch):
    import api.graphs as graphs
...
def test_list_shows_own_and_public_only(make_client, fake_db, seeded_entities):
...
def test_get_public_graph_of_other_user_visible(make_client, seeded_entities):
...
def test_get_unknown_graph_404(client):
...
def test_patch_and_delete_own_graph(client, gid, fake_db):
    r = client.patch(f"/v1/graphs/{gid}", json={"visibility": "public", "name": "Renamed"})
...
def test_patch_other_users_graph_forbidden(make_client, fake_db, seeded_entities):
...
def test_add_node_by_entity_and_custom(client, gid, fake_db):
    ...
    r = client.post(f"/v1/graphs/{gid}/nodes", json={"custom_label": "Novel Kinase X"})
    assert r.json()["duplicate"] is False
    r = client.post(f"/v1/graphs/{gid}/nodes", json={"custom_label": "Novel Kinase X"})
    assert r.json()["duplicate"] is True
...
def test_add_edge_dedup_and_validation(client, gid, fake_db):
    a = client.post(f"/v1/graphs/{gid}/nodes", json={"entity_id": "sirt1"}).json()["id"]
    b = client.post(f"/v1/graphs/{gid}/nodes", json={"entity_id": "nad"}).json()["id"]
    ...
    c = client.post(f"/v1/graphs/{other}/nodes", json={"custom_label": "Z"}).json()["id"]
...
def test_remove_node_and_edge(client, gid, fake_db):
...
def test_analytics_metrics(client, gid):
    ids = {}
    for label, entity in (("SIRT1", "sirt1"), ("Resveratrol", "resveratrol"),
        ...
        ids[label] = client.post(f"/v1/graphs/{gid}/nodes", json=payload).json()["id"]
    client.post(f"/v1/graphs/{gid}/edges", json={
        "from_node": ids["Resveratrol"], "to_node": ids["SIRT1"], "relation": "activates"})
    client.post(f"/v1/graphs/{gid}/edges", json={
        "from_node": ids["SIRT1"], "to_node": ids["NAD"], "relation": "consumes"})
...
def test_path_by_label(client, gid):
    a = client.post(f"/v1/graphs/{gid}/nodes", json={"entity_id": "resveratrol"}).json()["id"]
    b = client.post(f"/v1/graphs/{gid}/nodes", json={"entity_id": "sirt1"}).json()["id"]
    client.post(f"/v1/graphs/{gid}/edges", json={
        "from_node": a, "to_node": b, "relation": "activates"})

    r = client.get(f"/v1/graphs/{gid}/path", params={"frm": "Resveratrol", "to": "SIRT1"})
    body = r.json()
    assert body["connected"] is True
    assert [p["label"] for p in body["path"]] == ["Resveratrol", "SIRT1"]
    assert body["hops"] == [{"from": "Resveratrol", "to": "SIRT1", "relation": "activates"}]


def test_path_disconnected(client, gid):
    client.post(f"/v1/graphs/{gid}/nodes", json={"custom_label": "Island A"})
    client.post(f"/v1/graphs/{gid}/nodes", json={"custom_label": "Island B"})
    r = client.get(f"/v1/graphs/{gid}/path", params={"frm": "Island A", "to": "Island B"})
    assert r.json()["connected"] is False
    assert r.json()["path"] == []


def test_path_unknown_node_404(client, gid):
...
def test_db_failure_becomes_500(client, fake_db):
