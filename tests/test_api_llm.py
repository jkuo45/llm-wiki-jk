...
class TestExtractTriples:
    @pytest.mark.asyncio
    async def test_extracts_and_sanitizes(self, llm_offline):
    ...
    @pytest.mark.asyncio
    async def test_handles_markdown_fences(self, llm_offline):
        llm_offline["reply"] = (
            '```json\n{"triples": [{"subject": "A", "predicate": "inhibits", '
            '"object": "B", "confidence": 0.4, "rationale": "r"}]}\n```'
        )
        triples = await llm.extract_triples("t", "title", "abstract")
        assert triples == [{"subject": "A", "predicate": "inhibits",
                            "object": "B", "confidence": 0.4, "rationale": "r"}]

    @pytest.mark.asyncio
    async def test_no_triples_in_reply(self, llm_offline):
        llm_offline["reply"] = "I cannot extract anything from this."
        assert await llm.extract_triples("t", "title", "abstract") == []

    @pytest.mark.asyncio
    async def test_malformed_json_returns_empty(self, llm_offline):
        llm_offline["reply"] = '{"triples": [ {"subject": "A", '
        assert await llm.extract_triples("t", "title", "abstract") == []

    @pytest.mark.asyncio
    async def test_llm_failure_returns_empty(self, llm_offline):
        llm_offline["fail"] = True
        assert await llm.extract_triples("t", "title", "abstract") == []

    @pytest.mark.asyncio
    async def test_empty_abstract_short_circuits(self, llm_offline):
