...
- `.staged.json` — legacy; previously held live browser uploads, but live
  uploads were removed. It is no longer written by the runtime; leave it alone
  (see Rules for reconciliation).
...
- **Always generate the thumbnail** — after appending the manifest entry, run
  `uv run --with pillow python -m scripts.tools.thumbnail` (Step 4b) so
...
- **Only touch `manifest.json`** — `.staged.json` is the legacy live-upload
  store and is no longer written (uploads were removed);
...
`web/public/data/topics.json`, which `scripts/triples/rebuild.py` auto-derives
...
**Notes API** (in `api/routers/notes.py`):
