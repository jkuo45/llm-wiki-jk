---
title: ALKBH5
description: AlkB family protein 5 (ALKBH5) is an RNA demethylase that promotes TFEB
  expression by demethylating its mRNA, thereby establishing a positive feedback axis
  with TFEB.
type: entity
created: 2026-07-04
updated: 2026-07-04
tags:
  - enzyme
aliases: [AlkB homolog 5, RNA demethylase]
---
# ALKBH5

AlkB family protein 5 (ALKBH5) is an RNA demethylase that promotes [[TFEB]] expression by demethylating its mRNA, thereby establishing a positive feedback axis with TFEB.

## Function

ALKBH5 is an m6A demethylase that removes [[m6A Modification]] from mRNA, acting in opposition to [[METTL3]]. It regulates RNA metabolism including splicing, export, and stability.
## Autophagy Connection

ALKBH5 modulates autophagy gene expression through m6A-dependent mechanisms. Its dysregulation affects [[ULK1]] and [[Beclin1]] transcript levels in cancer.

## Documents

List of documents that mention this entity

  - [[_document_ - From the regulatory mechanism of TFEB to its therapeutic implications - Cell Death Discovery|From the regulatory mechanism of TFEB to its therapeutic implications - Cell Death Discovery]]
    - Supplementation with the RNA demethylase AlkB family protein 5 (ALKBH5) can reverse this effect because ALKBH5 demethylates TFEB to promote its expression \[\].


## Connections

- [[TFEB]]: AlkB family protein 5 (ALKBH5) is an RNA demethylase that promotes TFEB expression by demethylating its mRNA, thereby...
- [[m6A Modification]]: ALKBH5 is an m6A demethylase that removes m6A Modification from mRNA, acting in opposition to [[METTL3]].
- [[METTL3]]: ALKBH5 is an m6A demethylase that removes [[m6A Modification]] from mRNA, acting in opposition to METTL3.
- [[ULK1]]: Its dysregulation affects ULK1 and [[Beclin1]] transcript levels in cancer.
- [[Beclin1]]: Its dysregulation affects [[ULK1]] and Beclin1 transcript levels in cancer.

## Linking Summary
- New links added: [[TFEB]]
- Suggested new entity notes to create: [[RNA Demethylase]]
- Strong connections to strengthen: [[ALKBH5]] ↔ [[TFEB]]
