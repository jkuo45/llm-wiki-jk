---
title: ATGL
description: 'ATGL (Adipose Triglyceride Lipase) is a key enzyme involved in cytosolic
  lipolysis, catalyzing the initial step of triglyceride breakdown in Lipid Droplets.'
type: entity
created: 2026-07-04
updated: 2026-07-04
tags:
  - Enzyme
  - autophagy
aliases: [adipose triglyceride lipase, PNPLA2]

---

# ATGL

**ATGL** (Adipose Triglyceride Lipase) is a key enzyme involved in cytosolic lipolysis, catalyzing the initial step of triglyceride breakdown in [[Lipid Droplet]]s.

## Function and Regulation
- **Cytosolic Lipolysis**: ATGL localizes to the surface of lipid droplets and hydrolyzes triglycerides into diacylglycerols and free fatty acids.
- **Independence from Lipophagy**: In [[C. elegans]], ATGL (encoded by *atgl-1*) acts in parallel to the [[Lipophagy]] pathway involving [[LIPL-1]] and [[LIPL-3]].
- **Nutrient Status**: Activated during [[Starvation]] and [[Fasting]] to mobilize energy reserves.

## Function

ATGL (PNPLA2) catalyzes the first step of triglyceride lipolysis. Its activity provides substrate for [[Lipophagy]] and fatty acid oxidation.

## Connections

- [[Lipid Droplet]]: **ATGL** (Adipose Triglyceride Lipase) is a key enzyme involved in cytosolic lipolysis, catalyzing the initial step o...
- [[C. elegans]]: - **Independence from Lipophagy**: In C. elegans, ATGL (encoded by *atgl-1*) acts in parallel to the [[Lipophagy]] pa...
- [[Lipophagy]]: elegans]], ATGL (encoded by *atgl-1*) acts in parallel to the Lipophagy pathway involving [[LIPL-1]] and [[LIPL-3]].
- [[LIPL-1]]: elegans]], ATGL (encoded by *atgl-1*) acts in parallel to the [[Lipophagy]] pathway involving LIPL-1 and [[LIPL-3]].
- [[LIPL-3]]: elegans]], ATGL (encoded by *atgl-1*) acts in parallel to the [[Lipophagy]] pathway involving [[LIPL-1]] and LIPL-3.
- [[Starvation]]: - **Nutrient Status**: Activated during Starvation and [[Fasting]] to mobilize energy reserves.
- [[Fasting]]: - **Nutrient Status**: Activated during [[Starvation]] and Fasting to mobilize energy reserves.

## Linking Summary
- New links added: [[Lipid Droplet]], [[C. elegans]], [[Lipophagy]], [[LIPL-1]], [[LIPL-3]], [[Starvation]], [[Fasting]]
- Suggested new entity notes to create: [[Diacylglycerol]]
- Strong connections to strengthen: [[ATGL]] ↔ [[Lipid Droplet]]
