---
title: Foxp3
description: 'Foxp3 (Forkhead Box P3) is a transcription factor that serves as
  a master regulator of the regulatory T Cell (Treg) lineage.'
type: entity
created: 2026-07-04
updated: 2026-07-04
tags:
  - gene
aliases: [forkhead box P3]

---

# Foxp3

**Foxp3** (Forkhead Box P3) is a transcription factor that serves as a master regulator of the regulatory [[T Cell]] (Treg) lineage.

## Induction by Spermidine
- [[Spermidine]] can promote the differentiation of CD4+ T cells towards a regulatory Foxp3+ phenotype.
- This phenotype exhibits an anti-inflammatory profile.

## Autophagy Dependence
- The induction of the Foxp3+ phenotype is abrogated in **[[Atg5]]**-deficient T cells, which are incapable of [[Autophagy]] initiation.

## Function

Foxp3 is a forkhead transcription factor that specifies [[Regulatory T Cell]] (Treg) lineage. [[Autophagy]] maintains Treg metabolic fitness and suppressive function.

## Connections

- [[T Cell]]: **Foxp3** (Forkhead Box P3) is a transcription factor that serves as a master regulator of the regulatory T Cell (Tre...
- [[Spermidine]]: ## Induction by Spermidine - Spermidine can promote the differentiation of CD4+ T cells towards a regulatory Foxp3+ p...
- [[Atg5]]: ## Autophagy Dependence - The induction of the Foxp3+ phenotype is abrogated in **Atg5**-deficient T cells, which are...
- [[Autophagy]]: ## Autophagy Dependence - The induction of the Foxp3+ phenotype is abrogated in **[[Atg5]]**-deficient T cells, which...
- [[Regulatory T Cell]]: Foxp3 is a forkhead transcription factor that specifies Regulatory T Cell (Treg) lineage.

## Linking Summary
- New links added: [[T Cell]], [[Spermidine]], [[Atg5]], [[Autophagy]]
- Suggested new entity notes to create:
- Strong connections to strengthen: [[Foxp3]] ↔ [[Spermidine]]
