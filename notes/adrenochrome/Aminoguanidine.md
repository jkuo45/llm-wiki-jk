---
title: Aminoguanidine
description: Aminoguanidine is a chemical compound with the formula $CH_6N_4$. It
  is used as a reagent to stabilize Adrenochrome by forming Adrenochrome monoaminoguanidine.
type: entity
created: 2026-07-04
updated: 2026-07-04
tags:
  - Chemical Compound
aliases: []
---
# Aminoguanidine

Aminoguanidine is a chemical compound with the formula $CH_6N_4$. It is used as a reagent to stabilize [[Adrenochrome]] by forming [[Adrenochrome monoaminoguanidine]].

It was the first identified inhibitor of [[Advanced Glycation End Products]] (AGEs). It acts as a "carbonyl trap" by interacting with highly reactive carbonyl species, preventing them from forming cross-links with proteins.

## Therapeutic Roles and Safety

Aminoguanidine has been shown to prevent diabetic renal, retinal, and neural complications in experimental models. However, it is not in clinical use due to safety concerns, including:
- Pro-oxidant activities.
- Inhibition of nitric oxide (NO) synthase.
- Potential DNA damage by free radical production.

## Connections
- [[Adrenochrome]]: Stabilized by aminoguanidine to form [[Adrenochrome monoaminoguanidine]].
- [[Adrenochrome monoaminoguanidine]]: The stable derivative formed.
- [[Hemostatics]]: The resulting derivative is used as a hemostatic.
- [[Advanced Glycation End Products]]: Inhibited by aminoguanidine.
- [[Glycation]]: Prevented by its action as a carbonyl trap.

## Linking Summary
- New links added: [[Advanced Glycation End Products]], [[Glycation]]
- Strong connections to strengthen: [[Aminoguanidine]] ↔ [[Advanced Glycation End Products]]
