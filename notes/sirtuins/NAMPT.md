---
type: entity
category: enzyme
aliases: [nicotinamide phosphoribosyltransferase, visfatin, PBEF, pre-B-cell colony-enhancing factor]
database_ids:
  uniprot: P43490
  hgnc: HGNC:8683
  mesh: D015124
relations:
  - predicate: regulates
    target: "[[NAD+]]"
    sources: [doi:10.1016/j.cmet.2016.06.014]
    created: 2026-07-03
    updated: 2026-07-03
  - predicate: activates
    target: "[[SIRT1]]"
    sources: [doi:10.1016/j.cmet.2016.06.014]
    created: 2026-07-03
    updated: 2026-07-03
---

# NAMPT (Nicotinamide Phosphoribosyltransferase)

**Nicotinamide Phosphoribosyltransferase** (NAMPT) is the rate-limiting enzyme in the nicotinamide adenine dinucleotide ([[NAD+]]) salvage pathway in mammals. It converts nicotinamide (NAM) and 5-phosphoribosyl-1-pyrophosphate (PRPP) into nicotinamide mononucleotide ([[Nicotinamide Mononucleotide|NMN]]), which is then converted into NAD⁺ by NMN adenylyltransferases ([[NMNAT]]).

NAMPT plays a critical role in cellular metabolism, energy production, cell survival, and genomic stability by regulating the availability of NAD⁺, the obligate co-substrate for sirtuins (especially [[SIRT1]] and [[SIRT3]]) and PARPs (such as [[PARP1]]).

## Forms and Subcellular Localization

NAMPT exists in two distinct functional forms in mammals:
1. **Intracellular NAMPT (iNAMPT):** Localized in the cytoplasm and nucleus, iNAMPT is vital for maintaining cellular NAD⁺ pools. It is regulated by the circadian clock (via CLOCK:BMAL1 heterodimers binding to its promoter) to drive rhythmic NAD⁺ synthesis.
2. **Extracellular NAMPT (eNAMPT / Visfatin / PBEF):** Secreted primarily by adipose tissue, immune cells, and hepatocytes. eNAMPT acts as an adipokine and cytokine with pro-inflammatory and systemic insulin-mimetic effects, binding to receptors such as Toll-like receptor 4 ([[Toll-like Receptor|TLR4]]).

## Regulation of Sirtuin Activity

By controlling NMN synthesis and subsequently cellular NAD⁺ levels, NAMPT acts as a primary upstream master regulator of sirtuin-mediated deacetylation.
- **AMPK Activation:** During energetic stress, activation of [[notes/_link/AMPK|AMPK]] phosphorylates and activates NAMPT, elevating NAD⁺ levels to boost SIRT1 activity.
- **Resveratrol Indirect Axis:** Resveratrol-mediated PDE4 inhibition leads to accumulation of [[cAMP]], triggering calcium release and AMPK activation, which upregulates NAMPT to stimulate sirtuin activity.

## Role in Aging and Disease

- **Age-Related Decline:** Systemic and tissue-specific NAMPT expression declines with age, leading to NAD⁺ depletion, mitochondrial dysfunction, and diminished sirtuin activity (a key driver of the [[notes/_link/Hallmarks of Aging|Hallmarks of Aging]]).
- **Metabolic Syndrome:** Decreased NAMPT activity is associated with insulin resistance, obesity, and type 2 [[notes/_link/Diabetes Mellitus|diabetes]]. Conversely, NAMPT overexpression or NMN supplementation improves glucose tolerance and lipid profiles in animal models.
- **Cancer Biology:** Many cancer cells overexpress NAMPT to satisfy their high energetic and DNA repair demands (via PARPs). NAMPT inhibitors (e.g., FK866) are being investigated as anti-cancer agents to selectively deplete NAD⁺ and induce apoptosis in tumor cells.

## Connections

- **[[NAD+]]**: The downstream product and physiological target of NAMPT enzymatic activity.
- **[[SIRT1]]**: Primary nuclear deacetylase whose activity is highly sensitive to NAMPT-mediated NAD⁺ levels.
- **[[notes/_link/AMPK|AMPK]]**: Key metabolic sensor that upregulates NAMPT to restore cellular energy charge.
- **[[Nicotinamide Mononucleotide|NMN]]**: The direct enzymatic product of NAMPT.

## Linking Summary:
- New links added: [[NAD+]], [[SIRT1]], [[notes/_link/AMPK]], [[Nicotinamide Mononucleotide]], [[NMNAT]], [[PARP1]], [[Toll-like Receptor]], [[notes/_link/Hallmarks of Aging]], [[notes/_link/Diabetes Mellitus]], [[cAMP]]
- Suggested new entity notes to create: [[FK866]]
- Strong connections to strengthen: [[NAMPT]] ↔ [[NAD+]], [[NAMPT]] ↔ [[SIRT1]]
