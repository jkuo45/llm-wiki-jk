---
title: mtDNA
description: Mitochondrial DNA that can be released into the cytosol to activate innate immune signaling pathways like cGAS-STING.
type: entity
created: 2026-07-07
updated: 2026-07-07
tags:
  - biological-molecule
  - dna
  - mitochondria
  - inflammation
aliases: [mtDNA, mitochondrial DNA]
---

# mtDNA

**Mitochondrial DNA (mtDNA)** is the genetic material found in mitochondria. While primarily involved in编码 components of the electron transport chain, mtDNA can also act as a damage-associated molecular pattern (DAMP) when released into the cytosol.

## Release Mechanisms
mtDNA release can occur through:
- **Major Mitochondrial Outer Membrane Permeabilization (MOMP)**: Typically associated with [[Apoptosis]].
- **Minority MOMP (miMOMP)**: Sublethal permeabilization by [[BAX]]/[[BAK]] that releases mtDNA without triggering cell death.

## Role in Senescence
In [[Senescent Cells]], mitochondrial dysfunction and miMOMP lead to the accumulation of cytosolic mtDNA. This activates the [[cGAS-STING Pathway|cGAS-STING]] innate immune sensor, driving the production of type I interferons and pro-inflammatory cytokines, which are key components of the [[Senescence-Associated Secretory Phenotype|SASP]].

## Clinical Relevance
Cytosolic mtDNA and the resulting [[cGAS-STING Pathway|cGAS-STING]] activation are major drivers of sterile [[Inflammaging]] and age-related diseases.

## Documents

List of documents that mention this entity

  - [[_document_ - SASP, senescent cells, grok|SASP, senescent cells, grok]]
    - Explains how mtDNA release via miMOMP activates the cGAS-STING pathway to drive SASP.

## Connections

- [[cGAS-STING Pathway|cGAS-STING]] — Activates the cGAS-STING innate immune sensor
- [[Minority MOMP]] — Released into the cytosol via miMOMP
- [[Senescence-Associated Secretory Phenotype|SASP]] — Induces SASP production
- [[Senescence-associated mitochondrial dysfunction|SAMD]] — A consequence of mitochondrial dysfunction in senescence
- [[Inflammaging]] — Drives sterile inflammation in aging

## Linking Summary

  - New links added: [[cGAS-STING Pathway|cGAS-STING]], [[Minority MOMP]], [[Senescence-Associated Secretory Phenotype|SASP]], [[Senescence-associated mitochondrial dysfunction|SAMD]], [[Inflammaging]]
  - Strong connections to strengthen: [[mtDNA]] ↔ [[cGAS-STING Pathway|cGAS-STING]]
