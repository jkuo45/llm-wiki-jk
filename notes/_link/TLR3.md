---
title: TLR3
description: Toll-like receptor 3, an endosomal pattern-recognition receptor that binds double-stranded RNA and triggers antiviral/inflammatory signaling exclusively through the TRIF adaptor.
created: 2026-07-08
updated: 2026-07-08
tags:
  - receptor
  - innate-immunity
  - pattern-recognition-receptor
  - toll-like-receptor
aliases:
  - Toll-like receptor 3
  - CD283
---

# TLR3

## Overview

**TLR3 (Toll-like receptor 3)** is an endosomal [[Toll-like Receptor|toll-like receptor]] expressed on immune and epithelial cells that recognizes double-stranded RNA (dsRNA)—including viral replication intermediates and the synthetic analog poly(I:C). Unlike most TLRs, TLR3 signals exclusively through the [[TRIF]] (TICAM1) adaptor rather than MyD88.

## Structure & Localization

TLR3 is a type I transmembrane glycoprotein with an extracellular leucine-rich repeat (LRR) domain for ligand sensing and a cytoplasmic Toll/IL-1 receptor (TIR) domain that recruits TRIF. It resides primarily in endosomes/lysosomes where it encounters dsRNA internalized or generated during infection.

## Mechanism of Action & Signaling

1. dsRNA binding induces TLR3 dimerization and recruitment of [[TRIF]] via its TIR domain.
2. TRIF engages [[TRAF3]] → activates [[TBK1]]/IKKε → phosphorylates [[IRF3]] → type I interferon (IFN-β) production.
3. TRIF also activates [[RIPK1]]/[[TRAF6]] → [[NFκB|NF-κB]] and [[MAPK|MAPK]] pathways → pro-inflammatory cytokine expression ([[TNF-alpha|TNF-α]], [[IL-6]]).
4. The TLR3/TRIF axis can converge on the [[NFκB]]/[[STAT1]] axis driving inflammatory gene programs.

## Physiological Function

TLR3 is a frontline antiviral sensor initiating interferon and inflammatory responses. It also contributes to tissue responses to endogenous dsRNA-like signals during injury and sterile inflammation.

## Pathology & Clinical Relevance

- Excessive TLR3/TRIF signaling underlies viral encephalitis, autoimmune/inflammatory conditions, and exaggerated responses to tissue injury.
- In muscle and joint biology, poly(I:C)-induced TLR3 activation models inflammatory damage.

## Role in the Urolithin A Review

The review *Pharmacological Effects of Urolithin A and Its Role in Muscle Health and Performance* notes that [[Urolithin A]] inactivates [[TLR3]]/[[TRIF]] signaling to block the [[NFκB]]/[[STAT1]] axis, thereby reducing [[Inflammation|inflammation]] and enhancing [[Antioxidants|antioxidant]] defense (demonstrated in poly(I:C)-stimulated RAW264.7 macrophages).

## Documents

- [[_document_ - Pharmacological Effects of Urolithin A and Its Role in Muscle Health and Performance Current Knowledge and Prospects|Urolithin A and Muscle Health Review]]
  - Describes UA blockade of TLR3/TRIF → NF-κB/STAT1 as an anti-inflammatory mechanism.

## Connections

- [[TLR3]] → signals via → [[TRIF]]
- [[TRIF]] → activates → [[NFκB]], [[STAT1]], [[IRF3]]
- [[Urolithin A]] → inactivates → [[TLR3]] / [[TRIF]]

## Linking Summary

- New links added: [[Toll-like Receptor]], [[TRIF|TICAM1]], [[TRAF3]], [[TBK1]], [[IRF3]], [[RIPK1]], [[TRAF6]], [[NFkB]], [[MAPK]], [[STAT1]], [[TNF-alpha]], [[IL-6]], [[Urolithin A]], [[Inflammation]], [[Antioxidants]]
- Suggested new entity notes to create: none (TICAM1 is an alias of [[TRIF]])
- Strong connections to strengthen: [[Urolithin A]] ↔ [[TLR3]]
