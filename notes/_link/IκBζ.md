---
title: IκBζ
description: Inhibitor of NF-κB zeta, a selective co-activator of NF-κB for SASP genes like IL-6 and IL-8.
type: entity
created: 2026-07-07
updated: 2026-07-07
tags:
  - transcription-factor
  - sasp
  - inflammation
---

# IκBζ

IκBζ (Inhibitor of NF-κB zeta) is a selective co-activator of [[NF-κB]] for a subset of target genes, including _IL6_ and _IL8_. It is a major regulator of SASP genes.

## Mechanism of Action

- IκBζ is a co-activator of [[NF-κB]]
- It selectively activates a subset of NF-κB target genes
- IκBζ is essential for the expression of [[IL-6]] and [[IL-8]]
- IκBζ is a key regulator of SASP gene expression

## Role in SASP

- IκBζ is a master regulator of SASP interleukins
- It is required for IL-6 and IL-8 expression
- IκBζ links NF-κB signaling to SASP composition

## Clinical Relevance

- IκBζ is a therapeutic target in inflammatory diseases
- IκBζ may be a target for modulating SASP
- IκBζ is implicated in age-related inflammation

# 

## Documents

- [[_document_ - The-senescence-associated-secretory-phenotype-and-its-physiological-and-pathological-implications|SASP Review 2024]]: The review highlights IκBζ as a selective co-activator of NF-κB required for expression of the SASP interleukins IL-6 and IL-8. By linking NF-κB signaling to the specific composition of the secretome, it is implicated as a master regulator of the inflammatory SASP arm and a potential target for modulating senescence-associated inflammation.

## Connections

- [[NF-κB]]: IκBζ is a co-activator of NF-κB
- [[IL-6]]: IκBζ regulates IL-6 expression
- [[IL-8]]: IκBζ regulates IL-8 expression
- [[Senescence-Associated Secretory Phenotype|SASP]]: IκBζ is a major SASP regulator

## Linking Summary

- New links added: [[NF-κB]], [[IL-6]], [[IL-8]], [[Senescence-Associated Secretory Phenotype|SASP]]
  - Strong connections to strengthen: [[IκBζ]] ↔ NF-κB, [[IκBζ]] ↔ SASP regulation
