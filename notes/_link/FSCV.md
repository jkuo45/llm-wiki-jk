---
type: entity
title: FSCV
description: '**Fast-scan cyclic voltammetry (FSCV)** is an electrochemical technique
  used to detect rapid changes in Neurotransmitter concentration in real time, typically
  in living brain tissue. It employs a c...'
created: 2026-07-04
updated: 2026-07-06
entity_type_1: Analytical Technique
aliases: [Fast-Scan Cyclic Voltammetry]
---
# FSCV

**Fast-scan cyclic voltammetry (FSCV)** is an electrochemical technique used to detect rapid changes in [[Neurotransmitter]] concentration in real time, typically in living brain tissue. It employs a carbon-fiber microelectrode and a rapid voltage ramp (typically 400 V/s, −0.4 to +1.3 V) to oxidize and reduce electroactive species.

## Principle

A triangular voltage waveform is applied to a carbon-fiber [[Microelectrode]] at 10 Hz. Electroactive [[Neurotransmitters]] ([[Dopamine]], [[Serotonin]], [[Norepinephrine]], [[Adenosine]]) are oxidized at characteristic potentials, producing a [[Faradaic Current]] proportional to concentration. Background subtraction isolates the signal from the rapid voltage scan. The resulting [[Cyclic Voltammogram]] provides a chemical fingerprint for analyte identification.

## Applications

FSCV measures sub-second [[Dopamine]] release in awake, behaving animals (rodents). It is used to study [[Reward System]] function, [[Addiction]], [[Motivation]], and the effects of [[Psychostimulants]] ([[Cocaine]], [[Amphetamine]]). Recent advances enable [[Serotonin]] detection with modified waveforms.

#

## Connections
- [[Dopamine]] — primary analyte detected by FSCV in neuroscience
- [[Neurotransmitter]] — FSCV detects electroactive neurotransmitters
- [[Carbon-fiber Microelectrode]] — the recording sensor for FSCV
- [[Cyclic Voltammetry]] — the underlying electrochemical method
- [[Addiction]] — FSCV is used to study dopamine dynamics in addiction models
- [[Reward System]] — FSCV measures phasic dopamine release during reward processing
- [[Cocaine]] — effects on dopamine reuptake measured by FSCV
- [[Serotonin]] — can be detected with modified FSCV waveforms

## Linking Summary
- New links added: [[Dopamine]], [[Serotonin]], [[Norepinephrine]], [[Adenosine]], [[Neurotransmitter]], [[Carbon-fiber Microelectrode]], [[Cyclic Voltammetry]], [[Microelectrode]], [[Reward System]], [[Addiction]], [[Cocaine]], [[Amphetamine]], [[Motivation]], [[Psychostimulants]]
- Suggested new entity notes to create: [[Faradaic current]], [[Cyclic voltammogram]], [[Carbon-fiber electrode]], [[Background subtraction]], [[Electrochemical detection]], [[In vivo electrochemistry]]
- Strong connections to strengthen: [[FSCV]] ↔ [[Dopamine]], [[FSCV]] ↔ [[Addiction]]
