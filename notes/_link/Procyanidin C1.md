---
title: Procyanidin C1
description: A flavonoid (oligomeric proanthocyanidin) with senotherapeutic (senolytic/senomorphic) activity that extends healthspan and lifespan in mice.
created: 2026-07-10
updated: 2026-07-10
tags:
  - chemical-compound
  - flavonoid
  - senolytic
  - senomorphic
  - longevity
aliases: [Procyanidin C1, PCC1, Procyanidin C1 (PCC1)]
---

# Procyanidin C1

**Procyanidin C1 (PCC1)** is an oligomeric proanthocyanidin flavonoid shown to have senotherapeutic activity — selective senolysis of senescent cells and extension of healthspan and lifespan in mice[11, 28, 71].

## Relevance to Apigenin Study

> [!info] Source: [[_document_ - repurposing_apigen_senomorphic.09.09.611999v1.full|Repurposing apigenin for senomorphic effect in antiaging pipelines]]
> PCC1 served as a positive **senolytic** control (50 μM) in the natural-product library screen that identified [[Apigenin]] as a senomorphic. Unlike apigenin, PCC1 fell in the senolytic (selective-killing) region of the screen, illustrating the distinction between senolytic and [[Senomorphic|senomorphic]] classes of flavonoids.

## Documents

List of documents that mention this entity

  - [[_document_ - cellular_senescence_ipf_diseases-14-00201|Cellular Senescence in IPF (Baurzhan et al., 2026)]]
    - Procyanidin C1 is a senotherapeutic flavonoid that clears senescent cells (via BCL-2) and shows antifibrotic activity in animal models.

## Connections

- [[Senolytic]] — PCC1 is a canonical natural senolytic.
- [[Apigenin]] — Comparator; apigenin is senomorphic whereas PCC1 is senolytic.
- [[Senomorphic]] — Contrast class in the same screen.
- [[Senescence]] — PCC1 clears senescent cells.
- [[Senolytics]]: Procyanidin C1 is a senolytic acting via BCL-2.
- [[Idiopathic Pulmonary Fibrosis]]: Antifibrotic in pulmonary fibrosis models.

## Linking Summary

- New links added: [[Senolytic]], [[Apigenin]], [[Senomorphic]], [[Senescence]]
- Suggested new entity notes to create: (none)
- Strong connections to strengthen: [[Procyanidin C1]] ↔ [[Senolytic]]
