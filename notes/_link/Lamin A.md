---
title: Lamin A
description: Lamin A is a nuclear-envelope intermediate-filament protein encoded by LMNA; defective processing produces progerin and drives progeroid syndromes.
created: 2024-01-01
updated: 2026-07-09
tags:
  - protein
url: '#'
source: '#'
aliases: []
---

# Lamin A

**Lamin A** is a type-V intermediate-filament protein that, together with lamin C and the B-type lamins, forms the **nuclear lamina** — a meshwork lining the inner nuclear membrane. It is encoded by the [[LMNA Gene]] and provides mechanical support to the [[Nucleus|nucleus]], anchors the [[Nuclear Pore Complex]], and organizes [[Chromatin]] into lamina-associated domains (LADs) that are typically transcriptionally silent [[Heterochromatin]].

## Structure & Processing

- Synthesized as **prelamin A**, which is farnesylated, then cleaved by the protease ZMPSTE24 to remove the farnesylated C-terminus, yielding mature lamin A.
- Failure of this final cleavage traps a permanently farnesylated, truncated form at the nuclear membrane.

## Pathology & Clinical Relevance

> [!important] Progerin and progeria
> A silent point mutation in [[LMNA Gene|LMNA]] (c.1824C>T) activates a cryptic splice site, producing **[[Progerin]]** — a toxic lamin A that cannot be de-farnesylated. Progerin accumulation causes [[Hutchinson-Gilford Progeria Syndrome|HGPS]] ([[HGPS]]), characterized by nuclear blebbing, loss of [[Heterochromatin]] (reduced H3K9me3, HP1), [[Genomic Instability]], and premature [[Cellular Senescence]].

Notably, low levels of progerin also accumulate during **normal [[Aging]]**, linking lamin A biology to physiological aging and making the nuclear lamina a model for studying [[Epigenetic Alterations]] and heterochromatin loss. Other LMNA mutations cause laminopathies including dilated cardiomyopathy and muscular dystrophies.

## Documents

  - [[_document_ - Epigenetic changes during aging and their reprogramming potential|Epigenetic Changes During Aging]]
    - Cites reduced global heterochromatin (HP1, H3K9me3) and lamin A changes as conserved aging markers across model organisms.

## Connections

  - [[HGPS]]: Progeroid syndrome caused by progerin.
  - [[Progerin]]: Toxic truncated lamin A isoform.
  - [[LMNA Gene]]: Gene encoding lamin A/C.
  - [[Aging]]: Normal aging shows low-level progerin accumulation.
  - [[Chromatin]]: Organized at the lamina into LADs.
  - [[Heterochromatin]]: Lost as lamina integrity declines.
  - [[Genomic Instability]]: Downstream of lamina disruption.

## Linking Summary

  - New links added: [[HGPS]], [[Progerin]], [[LMNA Gene]], [[Aging]], [[Chromatin]], [[Heterochromatin]], [[Nuclear Pore Complex]], [[Genomic Instability]], [[Cellular Senescence]], [[Epigenetic Alterations]]
  - Strong connections to strengthen: Lamin A ↔ Progerin; Lamin A ↔ HGPS; Lamin A ↔ Heterochromatin
