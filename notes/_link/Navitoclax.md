---
title: Navitoclax
description: Navitoclax (ABT-263) is an orally bioavailable BH3-mimetic that inhibits BCL-2/BCL-xL/BCL-w, acting as a senolytic and anticancer agent limited by thrombocytopenia.
created: 2026-07-04
updated: 2026-07-09
tags:
  - chemical-compound
  - senolytic
  - pharmacology
url: '#'
source: '#'
aliases: [ABT-263]
---

# Navitoclax

**Navitoclax (ABT-263)** is a small-molecule **BH3-mimetic** that promotes apoptosis by antagonizing anti-apoptotic BCL-2 family proteins.

## Chemical Properties & Mechanism of Action

- **Target selectivity:** binds the hydrophobic BH3 groove of [[Bcl-2]], BCL-xL, and BCL-w with high affinity, displacing pro-apoptotic effectors [[BAX]]/[[BAK]] and sensitizer BH3-only proteins.
- **Downstream effect:** freed BAX/BAK oligomerize, triggering [[Mitochondrial outer membrane permeabilization]], [[Cytochrome C]] release, and caspase-mediated [[Apoptosis]].
- Orally bioavailable derivative of the tool compound ABT-737.

## Applications

> [!info] Senolytic activity
> Senescent cells resist apoptosis through upregulated **senescent-cell anti-apoptotic pathways (SCAPs)**, notably BCL-xL. By inhibiting BCL-xL, navitoclax selectively kills many [[Senescent Cells]], making it a prototypical [[Senolytics|senolytic]] used to clear [[Cellular Senescence]] and reduce the [[SASP|Senescence-Associated Secretory Phenotype]] in aging and fibrotic disease models. It also shows antitumor activity in lymphoid malignancies.

> [!warning] Dose-limiting toxicity
> Because platelets depend on BCL-xL for survival, navitoclax causes on-target **[[Thrombocytopenia]]**, which limits clinical dosing and has motivated development of more selective or targeted senolytics.

## Documents

  - [[_document_ - Fisetin is a senotherapeutic that extends health and lifespan|Fisetin as a Senotherapeutic]]
    - References navitoclax as a benchmark senolytic BH3-mimetic against which flavonoid senolytics such as fisetin are compared.
  - [[_document_ - SASP, senescent cells, grok|SASP & Senescent Cells]]
    - Discusses navitoclax's targeting of BCL-2/BCL-xL survival pathways to eliminate senescent cells and suppress the SASP.

## Connections

  - [[Senolytics]]: Navitoclax is a canonical senolytic.
  - [[Senescent Cells]]: Selectively eliminated by BCL-xL inhibition.
  - [[Bcl-2]]: Primary anti-apoptotic target family.
  - [[Apoptosis]]: Reactivated by BH3 mimicry.
  - [[SASP|Senescence-Associated Secretory Phenotype]]: Reduced by clearing senescent cells.
  - [[Thrombocytopenia]]: Dose-limiting on-target toxicity.

## Linking Summary

  - New links added: [[Bcl-2]], [[BAX]], [[BAK]], [[Apoptosis]], [[Mitochondrial outer membrane permeabilization]], [[Cytochrome C]], [[Senolytics]], [[Senescent Cells]], [[Cellular Senescence]], [[SASP|Senescence-Associated Secretory Phenotype]], [[Thrombocytopenia]]
  - Strong connections to strengthen: Navitoclax ↔ Senolytics; Navitoclax ↔ Bcl-2
