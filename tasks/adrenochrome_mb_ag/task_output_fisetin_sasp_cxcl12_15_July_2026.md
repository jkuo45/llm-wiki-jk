...
- **Fisetin ↔ CXCL12**: Senolytic normalizes a single circulating SASP factor to rescue endothelial function — a "humoral senescence" mechanism
...
- **Endothelial-to-Mesenchymal Transition ↔ Endothelial Dysfunction**: Transdifferentiation (loss of CDH5/PECAM1, gain of TGFB1/ACTA2) is a novel mechanism of age-related vascular dysfunction driven by the SASP
...
**Mechanism:** CXCL12 is a circulating SASP factor that directly impairs endothelial function. The Mahoney et al. 2025 paper (Mahoney et al. 2025) demonstrates that:
...
**Evidence strength:** Direct EXTRACTED reference edge (confidence: EXTRACTED)
...
```
This INFERRED edge represents the biochemical bridge: NO scavenging by superoxide reduces bioavailability, which further impairs mitochondrial function — a feed-forward loop.
...
**The EndoMT node connects to:**
- CDH5, PECAM1, TGFB1, ACTA2 — all EXTRACTED edges from the Mahoney paper
- Endothelial Dysfunction — the downstream consequence
...
- CXCL12 signaling through CXCR4/ACKR3 activates p21 and p16 in neighboring cells
...
| **EndoMT** | Blocks transdifferentiation | CDH5 ↑45%, PECAM1 ↑55%, TGFB1/ACTA2 ↓5.4-fold |
...
- **Vascular biology:** Endothelial Cells, Endothelial Dysfunction, EndoMT
- **Receptor system:** CXCR4, ACKR3
- **Structural markers:** CDH5, PECAM1, TGFB1, ACTA2
...
3. The combination of fisetin + LIT-927 targets both the source (senescent cells) and the mediator (CXCL12)
...
1. **How does the CXCR4-to-ACKR3 receptor switch reinforce the senescent phenotype?**
   - Crosses ACKR3, CXCR4, and Senescence communities
   - The INFERRED edge from fisetin to both receptors suggests fisetin may modulate this switch

2. **What is the mechanism by which fisetin reverses EndoMT in senescent endothelial cells?**
   - Crosses Fisetin, EndoMT, and SASP communities
   - The TGFB1/ACTA2 upregulation and CDH5/PECAM1 downregulation suggest TGF-β pathway involvement

3. **How does CXCL12 drive endothelial dysfunction through NO suppression and mitochondrial ROS?**
   - **This is the current trace** — answered above
   - Crosses CXCL12-Vascular Aging Axis, ROS, and Endothelial Dysfunction communities

4. **What is the therapeutic window for fisetin + LIT-927 combination therapy?**
   - The Mahoney paper shows the combination is more effective than either alone
   - The Bcl-2 transfection experiment confirms senolytic mechanism
   - Dosing optimization and timing relative to senescent cell burden are open questions

---

## Data Sources

- **Primary source:** Mahoney et al. 2025 — "Senolytic Treatment With Fisetin Reverses Age-Related Endothelial Dysfunction Partially Mediated by SASP Factor CXCL12"
- **Graph:** `graphify-out/graph.json` (1,850 nodes, 2,388 edges)
- **Communities:** 351 detected, 40 custom-labeled
- **Extraction:** Semantic extraction from 10 changed document files (0 code files)
- **Graphify update:** Incremental build on 2026-07-15
...
- Fisetin: Critical bridge node (8 edges) connecting SASP community to vascular dysfunction community
- SASP: Upstream driver (62 edges) of all downstream pathology
- Endothelial Dysfunction: Downstream phenotype (9 edges) — the clinical target
- Nitric Oxide: Lost protective mediator — restored by fisetin, suppressed by CXCL12
- Mitochondria: Source of superoxide — 2.2-fold increase from CXCL12, 58% reduction by fisetin
- EndoMT: Transdifferentiation mechanism — blocked by fisetin
