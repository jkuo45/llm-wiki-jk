...
def main():
    parser = argparse.ArgumentParser(description="Check if README.md is in sync with entities in topic directories.")
    parser.add_argument("--topic", help="Specific topic directory to audit (e.g., notes/adrenochrome)")
    parser.add_argument("--all", action="store_true", help="Audit all topic directories in notes/")
    ...
    if args.topic:
        ...
        topics = [os.path.join(notes_dir, d) for d in os.listdir(notes_dir) 
