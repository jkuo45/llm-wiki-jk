...
Run:  python3 scripts/normalize_triples_schema.py [--check]
"""
...
def normalize_context(t: dict) -> dict:
    ...
    if isinstance(c, dict):
        ...
        return {"en-US": (ctx["en-US"] or "").strip(), "zh-TW": (ctx["zh-TW"] or "").strip()}
...
def main() -> int:
    ...
    if problems:
        ...
        for k in ("missing_zh", "missing_en", "missing_dates", "bad_timestamp",
                  "updated_before_created", "duplicate_id", "missing_subject",
                  "missing_predicate", "missing_object", "non_object_triple"):
            if problems[k]:
