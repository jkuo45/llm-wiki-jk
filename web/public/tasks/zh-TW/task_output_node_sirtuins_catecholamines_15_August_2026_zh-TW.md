...
> 方法：`uv run --with networkx --with scipy python3 scripts/04_node_analysis.py --sources sirt1 sirt3 sirt2 comt mao --targets dopamine epinephrine`
...
- 指令：`uv run --with networkx --with scipy python3 scripts/04_node_analysis.py --sources sirt1 sirt3 sirt2 comt mao --targets dopamine epinephrine`（預設 `--seed 1`）。
- 無向投影；僅取巨成分；移除自環。
