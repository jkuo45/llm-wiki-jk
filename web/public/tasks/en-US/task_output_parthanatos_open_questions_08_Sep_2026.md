...
- Metabolic-state determinants:
  1. **Glycolysis vs OXPHOS reliance**: glycolysis-reliant cells (neurons, LN428, astrocytes in glucose-only aCSF) die faster via PAR–HK1 block; pyruvate/glutamine-capable cells bypass to mitochondria and resist (Zong 2004; Andrabi 2014; Fouquerel 2014).
  2. **Carbon source in medium**: glucose-only unmasks HK block; +pyruvate/+glutamine masks it — explains Alano (astrocytes, NAD-rescuable) vs Andrabi/Fouquerel (neurons/glioblastoma, NAD-insufficient) discrepancy.
  3. **Redox/PPP state**: HK block starves PPP → NADPH/GSH fall; GSH-ester/NADPH support rescues independent of ATP (Hossain 2024; Andrabi Fig.5E).
  4. **TRPM2/Ca2+ wiring**: TRPM2-high excitable cells (striatal/cortical/hippocampal neurons, cardiomyocytes, β-cells, microglia) use Ca2+→calpain→BID–BAX→AIF arm; low-TRPM2/MNNG systems use ER-store or Ca2+-independent routes (Bentle 2006; Munoz 2017; Zhong 2018).
  5. **Proliferation/quiescence**: G1-arrested/senescence-like BER-deficient cells accumulate toxic PAR → early OCR fall without ECAR compensation → late AIF (ResearchSquare rs.3.rs-10223032/v1 Jul 2026); ATM-KO PARGi death is replication-stress, AIF-negative (J Transl Med 2026).
  6. **Mito pool**: SNpc DA neurons show mito-PAR + VDAC1 colocalization adjacent to Lewy bodies (Cell Death Dis 2026) — postmitotic neurons pre-positioned for outer-membrane AIF-pool release.
- Direction: minimal cell-state panel — (a) lineage + TRPM2 isoform/expression, (b) ECAR/OCR + fuel (glucose ± pyruvate/glutamine), (c) NAD/PAR/ATP same-well, (d) AIF IF + subfraction + rescue, (e) PARG53/ARH3 status. Test pyruvate + GSH-ester vs MIF-inhibitor vs TRPM2-KO in the same line to assign dominant arm.
